# How to use the code or try out:
## BASELINE CODE:
...
and what is the individual contribution of various linguistic features to its performance?
...
Our baseline model is a basic (single-layer) Perceptron classifier. This model takes a concatenation of several hand-crafted linguistic features as input and outputs a binary classification. The Perceptron was trained using a step
...
1. Comparative Prompting (Strategy A):

In this setup, both candidate hypotheses (H1 and H2) were presented simultaneously within a single input prompt, and
...
4. Individual Assessment Prompting
(Strategy B):

Here, each hypothesis was
...
WA and WB after lowercasing and tokenization, the Jaccard similarity J(SA, SB )
